...
namespace Rest560SpecV1
{
    ...
    class DisjunctiveDoubleFilteredTableSpec : Spec
    {
        ...
        public HashSet<(int Col,HashSet<int>[] PosRows)> extractPossibleSingularLastColumnMappings(State state, object obj) {
            var result = new HashSet<(int Col,HashSet<int>[] PosRows)>();
            foreach (var settle in GetSettlePoints(state, obj)) {
                int rowcount = settle.GetLength(0);
                int colcount = settle.GetLength(1);
                HashSet<int>[] vline = null;
                // Console.Out.WriteLine("here's achieved amount: {0}",colcount-1);

                if (1==colcount) {
                    vline = new HashSet<int>[rowcount];
                    for (int j=0;j<rowcount;j++) {
                        var slot = new HashSet<int>();
                        foreach ((int Row, int Col) in settle[j,0]) slot.Add(Row);
                        vline[j] = slot;
                    }
                }
                foreach ((int Row, int Col) in settle[0, colcount - 1]) result.Add((Col,vline));
            }
            return result;
        }
            ...
            for (int columnwiseIndex = 0; columnwiseIndex < columnwiseGroup.Count; columnwiseIndex++)
            {
                ...
                foreach (var assignment in assignments)
                {
                    ...
                    if (GetSettlePoint(newinitial))
                    {
                        ...
                        {
                            var intersection = new HashSet<int>();//eliminates duplicates
                            foreach ((int Row, int Col) in newinitial[row, 0]) intersection.Add(Row);//THIS WAS CHANGED
                            ra.Add(intersection);
                        }
        ...
        public (List<string[][]>[],HashSet<int>[][]) addJoiningColumnToSpec(State state, object obj, int colId)
        {
            var reras = getRowAssignmentsToSatisfyRest(state, obj);
            var candidate = obj as List<string[]>;
            var result = new List<string[][]>[reras.Count];
            var rowassign = new HashSet<int>[reras.Count][];
            for (int ri = 0; ri < reras.Count; ri++) {
                var innerrowassign = new HashSet<int>[reras[ri].Count];
                for (int k=0;k<reras[ri].Count;k++) {
                    innerrowassign[k] = reras[ri][k].RowAssignments;
                }
                rowassign[ri] = innerrowassign;
        ...
        {
            var other = spec as DisjunctiveDoubleFilteredTableSpec;
            if (other == null) return false;
            if (CustomTableExamples[state].Length != other.CustomTableExamples[state].Length) return false;
            for (int v = 0; v < CustomTableExamples[state].Length; v++)
            {//each possibility
                if (CustomTableExamples[state][v].Count != other.CustomTableExamples[state][v].Count) return false;
                for (int u = 0; u < CustomTableExamples[state][v].Count; u++)
                {//each component
                    if (CustomTableExamples[state][v][u].Length != other.CustomTableExamples[state][v][u].Length) return false;
                    for (int w = 0; w < CustomTableExamples[state][v][u].Length; w++)
                    {//each option
                        if (!CustomTableExamples[state][v][u][w].SequenceEqual(other.CustomTableExamples[state][v][u][w])) return false;
                    }
                }
            }
            return true;
